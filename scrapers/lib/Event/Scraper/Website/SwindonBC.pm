...
#  ( eval $(perl -Mlocal::lib=/usr/src/perl/libs/events/perl5/) ; perl -Ilib -MEvent::Scraper::Website::SwindonBC -MData::Dumper -le'my $events = Event::Scraper::Website::SwindonBC->get_events; print Dumper($events)' ) | less
...
#    my $page_url = "http://www.swindon.gov.uk/events/Pages/eventslisting.aspx?d=$csharp_time";
    my $page_url = "http://ww1.swindon.gov.uk/events/Pages/eventslisting.aspx?d=$csharp_time";
    while (1) {
...
sub elucidate {
    ...
    state $formatter = DateTime::Format::Strptime->new(
        pattern => "%A %d %B %Y", 
        on_error => 'croak', 
...
#    print "Meeting: $meeting\n" if $meeting;
    ...
    my ($start, $end) = map { print "start-end parse_datetime: $_\n"; $formatter->parse_datetime($_) } @dates;
    $end ||= '';
#    print "Start: $start, End: $end\n";
    ...
    my $time = $venue->right;
    until ($time->as_text =~ m/\d([:.]\d\d)? ?[ap]m/i) {
        $time = $time->right;
        if (!$time) {
...
#        $time =~ /([a-zA-Z]+)?(?:\s?-\s?)?([a-zA-Z]+)?s?,?\s?(\d+)\.?(\d+)?[ap]m(?:\s?-\s?)?(\d+)?\.?(\d+)?(?:[ap]m)?/;
    ...
    if ($end_hour and lc $end_hour eq 'noon') {
        $end_hour = 12;
    }
