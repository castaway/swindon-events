...
#    my $page_url = 'http://www.swindon.gov.uk/events/Pages/eventslisting.aspx?c=Music';
    while (1) {
...
#    $tree->dump;
    ...
    if (exists $known_venues->{$venue_name}) {
        $ret->{venue} = $known_venues->{$venue_name};
    } else {
        die "Don't know info for SBC venue '$venue_name'";
    }
...
sub get_times {
    ...
    #          1pm-3pm
    #          2pm
    my ($start_day, $end_day, $start_hour, $start_min, $end_hour, $end_min) = 
#        $time =~ /([a-zA-Z]+)?(?:\s?-\s?)?([a-zA-Z]+)?s?,?\s?(\d+)\.?(\d+)?[ap]m(?:\s?-\s?)?(\d+)?\.?(\d+)?(?:[ap]m)?/;
    ...
    if($start_day && $end_day) {
        @on_days = grep { state $found = 1 if($_ eq $start_day);
                          my @ds = $found ? ($_) : ();
    ...
    while ($loop_date->ymd ne $end->ymd) {
        if(grep { $loop_date->day_name } @on_days) {
